...
function App() {
  ...
  async function downloadZipAssetFromLatestRelease(repoOwner: string, repoName: string) {
    ...
    if (!response.ok) {
      throw new Error(
        `Error fetching release information: ${response.status} ${response.statusText}`
      )
    }

    const octokit = new Octokit({
      appId: import.meta.env.GITHUB_APP_ID as string,
      privateKey: import.meta.env.GITHUB_PRIVATE_KEY as string,
      clientId: import.meta.env.GITHUB_CLIENT_ID as string,
      clientSecret: import.meta.env.GITHUB_CLIENT_SECRET as string
    })

    const release: GithubRelease = await octokit.request(
    ...
    const downloadDir = await resolve(
      await path.downloadDir(),
      `${repoOwner}-${repoName}`
    )
    ...
    for (const file of entries) {
      if (file.name.endsWith('.filter')) {
        await rename(
          await resolve(downloadDir, file.name),
          await resolve(
            await path.documentDir(),
            `My Games\\Path of Exile 2\\${file.name}`
          )
        )
      } else {
  ...
  async function downloadLatestFilter() {
    await downloadZipAssetFromLatestRelease('NeverSinkDev', 'NeverSink-PoE2litefilter')
  }

  return (
    <main>
      <div className="navbar bg-base-100">
        <a className="btn btn-ghost text-xl">POE2 Filters Manager</a>
        {downloading && <span className="loading loading-bars loading-md"></span>}
      </div>
      <section>
